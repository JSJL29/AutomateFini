class Automate:
    ...
    def determinisation(self):
                ...
                for symbole in self.alphabets:
                    ...
                    for sous_etat in etat_courant.split(','):  # On lit chaque sous-état des états assemblés.
                        for transition in self.transitions:  # On lit chaque transitions de l'automate
                            ...
                            # On compare si l'état de départ dé la transition courante correspond au sous-état de l'état actuel et que l'alphabet correspond également
                            if depart == sous_etat and alph == symbole:
        ...
        # Cas où epsilon est présent dans l'alphabet
            ...
            for etat in self.etats:
                fermetures_epsilon[etat] = self.fermeture_epsilon(
                    etat)  # On appelle la fonction permettant de trouver les fermetures epsilons
            # On définit l'état initial et on traite à partir de cet état en suivant sa fermeture epsilon
            ...
            while file_traitement:
                ...
                for symbole in self.alphabets:
                    if symbole != 'ε':  # On ignore les transitions avec epsilons
                        etats_suivants = set()
                        for sous_etat in etat_courant.split(','):  # On lit chaque sous-état des états assemblés.
                            for etat_epsilon in fermetures_epsilon[sous_etat]:  # On lit chaque état de la fermeture epsilon
                                for transition in self.transitions:  # On lit chaque transitions de l'automate
                                    depart, alph, arrivee = transition.split(',')
                                    # On compare si l'état de départ dé la transition courante correspond à l'état actuel dans la fermeture epsilon et que l'alphabet correspond également
                ...
                if self.table_transitions[ligne][colonne] == " ":
                    if not Poubelle_ajouté:
                        ...
                        for i in range(len(self.alphabets)):
                            self.transitions.append(Poubelle + ',' + self.alphabets[i] + ',' + Poubelle) # Création de transition de l'état "Poubelle" vers lui-même pour chaque symbole de l'alphabet.
                        Poubelle_ajouté = True
                    self.transitions.append(self.etats[ligne] + ',' + self.alphabets[colonne] + ',' + Poubelle)

        self.table_transitions = self.construire_table_transitions() # Mis à jour de la table de transition
        return
    ...
    def mot_reconnu(self, mot):
        ...
        if len(mot) != 0 :
            symbole_courant = mot[0]
        else :
            symbole_courant = ''
        pos = 0 # Position dans le mot
        while True:
            if pos == len(mot):
